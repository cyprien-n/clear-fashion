...
const selectByFavorite = document.querySelector('#favorite-select');
 ...
 * @return {Object}
 */
const fetchProducts = async (page = 1, size = 12,brand='all',reasonablyPriced=false,recentlyReleased=false,sort='price-asc',favoriteProds=false) => {
  try {
    ...
    if(favoriteProds){
      result = result.filter(product => favorites.includes(product.uuid) );
    }
...
selectShow.addEventListener('change', async (event) => {
  const products = await fetchProducts(currentPagination.currentPage, parseInt(event.target.value),selectBrand.value,selectReasonablyPriced.checked,selectRecentlyReleased.checked,selectSort.value,selectByFavorite.checked);
...
selectPage.addEventListener('change', async (event) => {
  ...
  const products = await fetchProducts(numberOfPage,currentPagination.pageSize,selectBrand.value,selectReasonablyPriced.checked,selectRecentlyReleased.checked,selectSort.value,selectByFavorite.checked);
...
selectBrand.addEventListener('change', async (event) => {
  const products = await fetchProducts(currentPagination.currentPage,currentPagination.pageSize,event.target.value,selectReasonablyPriced.checked,selectRecentlyReleased.checked,selectSort.value,selectByFavorite.checked);
...
selectReasonablyPriced.addEventListener('change',async(event)=>{
  const products = await fetchProducts(currentPagination.currentPage,currentPagination.pageSize,selectBrand.value,event.target.checked,selectRecentlyReleased.checked,selectSort.value,selectByFavorite.checked);
  setCurrentProducts(products);
  render(currentProducts, currentPagination);
})
}
...
if(selectRecentlyReleased){
  selectRecentlyReleased.addEventListener('change',async(event)=>{
    const products = await fetchProducts(currentPagination.currentPage,currentPagination.pageSize,selectBrand.value,selectReasonablyPriced.checked,event.target.checked,selectSort.value,selectByFavorite.checked);
    setCurrentProducts(products);
    render(currentProducts, currentPagination);
  })
  }
  ...
  if(selectByFavorite){
    selectByFavorite.addEventListener('change',async(event)=>{
      const products = await fetchProducts(currentPagination.currentPage,currentPagination.pageSize,selectBrand.value,selectReasonablyPriced,selectRecentlyReleased.checked,selectSort.value,event.target.checked);
      setCurrentProducts(products);
      render(currentProducts, currentPagination);
    })
    }
  ...
  selectSort.addEventListener('change', async (event) => {
    const products = await fetchProducts(currentPagination.currentPage,currentPagination.pageSize,selectBrand.value,selectReasonablyPriced.checked,selectRecentlyReleased.checked,event.target.value,selectByFavorite.checked);
